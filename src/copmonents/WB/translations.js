export const translations = {
  de: {
    board_name_placeholder: "Name des Whiteboards…",
    circle: "Kreis",
    click_to_toggle: "Klicken Sie zum Umschalten",
    click_to_zoom:
      "Klicke zum reinzoomen\nHalte die Umschalttaste und klicke zum herauszoomen",
    collaborative_whiteboard: "Kollaboratives Whiteboard",
    color: "Farbe",
    configuration: "Konfiguration",
    create_private_board: "Erstelle ein privates Whiteboard",
    ellipse: "Ellipse",
    eraser: "Radierer",
    grid: "Gitter",
    hand: "Hand",
    index_title: "Wilkommen bei WBO!",
    introduction_paragraph:
      'WBO ist ein <a href="https://github.com/lovasoa/whitebophir" title="Frei im Sinne von Redefreiheit, nicht Freibier. Diese Software wird unter der AGPL Lizenz veröffentlicht.">freies und quelloffenes</a> kollaboratives Online-Whiteboard das vielen Nutzern erlaubt gleichzeitig auf einem großen virtuellen Whiteboard zu zeichnen. Das Whiteboard wird in Echtzeit für alle Nutzer aktualisiert und sein Inhalt wird gespeichert. Es kann für verschiedenste Anwendungen genutzt werden, z.B. Kunst, Unterhaltung, Design, Unterricht und Lehre.',
    keyboard_shortcut: "Tastenkombination",
    loading: "Lädt",
    menu: "Menü",
    mousewheel: "Mausrad",
    mover: "Verschiebung",
    named_private_board_description:
      "Du kannst auch ein <strong>privates Whiteboard mit Namen</strong> mit einer benutzerdefinierten URL erstellen. Alle die den Namen kennen, können darauf zugreifen.",
    opacity: "Deckkraft",
    open_public_board: "Gehe zum öffentlichen Whiteboard",
    pencil: "Stift",
    private_board_description:
      "Du kannst ein <b>privates Whiteboard</b> mit einem zufälligen Namen erstellen, welches man nur mit seinem Link öffnen kann. Benutze dies wenn du private Informationen teilen möchtest.",
    public_board_description:
      " Das <b>öffentliche Whiteboard</b> kann von jedem geöffnet werden. Es ein fröhliches unorganisiertes Chaos wo du zusammen mit anonymen Fremden malen kannst. Alles dort ist vergänglich.",
    recent_boards: "Neueste Whiteboards",
    rectangle: "Rechteck",
    share_instructions:
      "Um mit jemanden zusammen an einem Whiteboard zu arbeiten teile einfach die jeweilige URL.",
    size: "Größe",
    square: "Quadrat",
    straight_line: "Gerade Linie",
    tagline:
      "Ein freies quelloffenes kollaboratives Zeichentool. Zeichnet eure Ideen zusammen auf WBO!",
    text: "Text",
    tools: "Werkzeuge",
    view_source: "Quellcode auf GitHub",
    "white-out": "Korrekturflüssigkeit",
    zoom: "Zoom",
  },
  en: {
    "White-out": "White-out",
    board_name_placeholder: "Name of the board…",
    circle: "Circle",
    click_to_toggle: "click to toggle",
    click_to_zoom: "Click to zoom in\nPress shift and click to zoom out",
    collaborative_whiteboard: "Collaborative whiteboard",
    color: "Color",
    configuration: "Configuration",
    create_private_board: "Create a private board",
    ellipse: "Ellipse",
    eraser: "Eraser",
    grid: "Grid",
    hand: "Hand",
    index_title: "Welcome to the free online whiteboard WBO!",
    introduction_paragraph:
      'WBO is a <a href="https://github.com/lovasoa/whitebophir" title="Free as in free speech, not free beer. This software is released under the AGPL license">free and open-source</a> online collaborative whiteboard that allows many users to draw simultaneously on a large virtual board. The board is updated in real time for all connected users, and its state is always persisted. It can be used for many different purposes, including art, entertainment, design and teaching.',
    keyboard_shortcut: "keyboard shortcut",
    loading: "Loading",
    menu: "Menu",
    mousewheel: "mouse wheel",
    mover: "Mover",
    named_private_board_description:
      "You can also create a <strong>named private board</strong>, with a custom URL, that will be accessible to all those who know its name.",
    opacity: "Opacity",
    open_public_board: "Go to the public board",
    pencil: "Pencil",
    private_board_description:
      "You can create a <b>private board</b> with a random name, that will be accessible only by its link. Use this if you want to share private information.",
    public_board_description:
      "The <b>public board</b> is accessible to everyone. It is a happily disorganized mess where you can meet with anonymous strangers and draw together. Everything there is ephemeral.",
    recent_boards: "Recent Boards",
    rectangle: "Rectangle",
    share_instructions:
      "To collaborate on a drawing in real time with someone, just send them its URL.",
    size: "Size",
    square: "Square",
    straight_line: "Straight line",
    tagline:
      "A free and open-source online collaborative drawing tool. Sketch new ideas together on WBO!",
    text: "Text",
    tools: "Tools",
    view_source: "Source code on GitHub",
    zoom: "Zoom",
  },
  es: {
    board_name_placeholder: "Nombre de la pizarra …",
    circle: "Círculo",
    click_to_toggle: "haga clic para alternar",
    click_to_zoom:
      "Haga clic para acercar, Pulse [Mayús] y haga clic para alejar",
    collaborative_whiteboard: "Pizarra colaborativa",
    color: "Color",
    configuration: "Configuration",
    create_private_board: "Crea una pizarra privada",
    ellipse: "Elipse",
    eraser: "Borrador",
    grid: "Cuadrícula",
    hand: "Mano",
    index_title: "¡Bienvenido a WBO!",
    introduction_paragraph:
      'WBO es una pizarra colaborativa en línea, <a href="https://github.com/lovasoa/whitebophir" title="libre como la libertad de expresión, no libre como una cerveza gratis. Este software se lanza bajo la licencia AGPL">libre y de Código abierto</a>, que permite a muchos usuarios dibujar simultáneamente en una gran pizarra virtual. La pizarra se actualiza en tiempo real para todos los usuarios conectados y su estado siempre es persistente. Se puede utilizar para muchos propósitos diferentes, incluyendo arte, entretenimiento, diseño y enseñanza.',
    keyboard_shortcut: "atajo de teclado",
    loading: "Cargando",
    menu: "Menú",
    mousewheel: "Rueda del Ratón",
    mover: "Desplazamiento",
    named_private_board_description:
      'También puede crear una <strong>pizarra privada dándole un nombre aleatorio o un nombre especifico</strong>, una <abbr title="tipo: https://wbo.ophir.dev/boards/el-código-que-te-parezca">URL personalizada</abbr>, que será accesible para todos aquellos que conozcan su nombre.',
    opacity: "Opacidad",
    open_public_board: "Ir a la pizarra pública",
    pencil: "Lápiz",
    private_board_description:
      "Puede crear una <b>pizarra privada</b> con un nombre aleatorio, al que solo se podrá acceder mediante su enlace. Úselo si desea compartir información privada.",
    public_board_description:
      "La <b>pizarra pública</b> es accesible para todos. Es un desastre felizmente desorganizado donde puedes reunirte con extraños anónimos. Todo lo que hay es efímero.",
    recent_boards: "Pizarras recientes",
    rectangle: "Rectángulo",
    share_instructions:
      'Para colaborar en un dibujo en tiempo real con alguien, simplemente envíele la <abbr title="un enlace tipo: https://wbo.ophir.dev/boards/el-codigo-de-tu-pizarra">URL</abbr> de la pizarra que ya creaste.',
    size: "Tamaño",
    square: "Cuadrado",
    straight_line: "Línea recta",
    tagline:
      "Una herramienta de dibujo colaborativa en línea gratuita y de código abierto. Esboce nuevas ideas en la pizarra colaborativa WBO !",
    text: "Texto",
    tools: "Herramientas",
    view_source: "Código fuente en GitHub",
    "white-out": "Blanqueado",
    zoom: "Zoom",
  },
  fr: {
    board_name_placeholder: "Nom du tableau…",
    circle: "Cercle",
    click_to_toggle: "cliquer pour changer",
    click_to_zoom:
      "Cliquez pour zoomer\nCliquez en maintenant la touche majuscule enfoncée pour dézoomer",
    collaborative_whiteboard: "Tableau blanc collaboratif",
    color: "Couleur",
    create_private_board: "Créer un tableau privé",
    ellipse: "Ellipse",
    eraser: "Gomme",
    grid: "Grille",
    hand: "Main",
    index_title: "Bienvenue sur le tableau blanc collaboratif WBO !",
    introduction_paragraph:
      'WBO est un logiciel <a href="https://github.com/lovasoa/whitebophir" title="voir le code sous license AGPL">libre et gratuit</a> de dessin collaboratif en ligne qui permet à plusieurs utilisateurs de collaborer simultanément sur un tableau blanc. Le tableau est mis à jour en temps réel pour tous les utilisateurs connectés, et reste disponible après votre déconnexion. Il peut être utilisé notamment pour l\'enseignement, l\'art, le design ou juste pour s\'amuser.',
    keyboard_shortcut: "raccourci clavier",
    loading: "Chargement",
    menu: "Menu",
    mousewheel: "molette de la souris",
    mover: "Déplacer un élément",
    named_private_board_description:
      "Vous pouvez aussi créer un <strong>tableau privé nommé</strong>, avec une adresse personnalisée, accessible à tous ceux qui en connaissent le nom.",
    opacity: "Opacité",
    open_public_board: "Ouvrir le tableau anonyme",
    pencil: "Crayon",
    private_board_description:
      "Vous pouvez créer un <b>tableau privé</b> dont le nom sera aléatoire. Il sera accessible uniquement à ceux avec qui vous partagerez son adresse. À utiliser lorsque vous voulez partager des informations confidentielles.",
    public_board_description:
      "Le <b>tableau anonyme</b> est accessible publiquement. C'est un joyeux bazar où vous pourrez rencontrer des étrangers anonymes, et dessiner avec eux. Tout ce que vous y inscrivez est éphémère.",
    recent_boards: "Tableaux récents",
    rectangle: "Rectangle",
    share_instructions:
      "Pour collaborer sur un tableau avec quelqu'un, envoyez-lui simplement son URL.",
    size: "Taille",
    square: "Carré",
    straight_line: "Ligne droite",
    tagline:
      "Logiciel libre pour collaborer en ligne sur un tableau blanc. Venez dessiner vos idées ensemble sur WBO !",
    text: "Texte",
    tools: "Outils",
    view_source: "Code source sur GitHub",
    "white-out": "Blanco",
  },
  hu: {
    "White-out": "Lefedő",
    board_name_placeholder: "Tábla neve…",
    circle: "Kör",
    click_to_toggle: "kattintson ide a be- és kikapcsolásához",
    click_to_zoom:
      "Kattintson ide a nagyításhoz.\nShift + kattintás a kicsinyítéshez",
    collaborative_whiteboard: "Együttműködési tábla",
    color: "Szín",
    configuration: "Beállítások",
    create_private_board: "Saját tábla létrehozása",
    ellipse: "Ellipszis",
    eraser: "Radír",
    grid: "Rács",
    hand: "Kéz",
    index_title: "Isten hozta a WBO ingyenes online tábláján!",
    introduction_paragraph:
      'A WBO egy <a href="https://github.com/lovasoa/whitebophir" title="Ingyenes, mint a szabad beszédben, nem ingyenes sör. Ez a szoftver a AGPL licenc alapján kerül kiadásra.">ingyenes és nyílt forráskódú</a> online együttműködési tábla, amely lehetővé teszi sok felhasználó számára, hogy egyidejűleg rajzoljon egy nagy virtuális táblán. Az alaplap valós időben frissül az összes csatlakoztatott felhasználó számára és állapota állandó. Különböző célokra felhasználható, beleértve a művészetet, a szórakoztatást, a tervezést és a tanítást.',
    keyboard_shortcut: "billentyűparancs",
    loading: "Betöltés folyamatban",
    menu: "Menü",
    mousewheel: "egérkerék",
    mover: "Mozgató",
    named_private_board_description:
      "Készíthet egy <strong>saját nevű táblát</strong> is, egyéni URL-címmel, amely mindenki számára elérhető, aki ismeri a nevét.",
    opacity: "Átlátszatlanság",
    open_public_board: "Nyilvános tábla megnyitása",
    pencil: "Ceruza",
    private_board_description:
      "Készíthet egy <b>saját táblát</b> véletlenszerű névvel, amely csak a linkjével lesz elérhető. Használja ezt, ha személyes adatokat szeretne megosztani.",
    public_board_description:
      "A <b>nyilvános tábla</b> mindenki számára elérhető. Ez egy boldog szervezetlen rendetlenség, ahol találkozhat a névtelen ismeretlenek és dolgozhat együtt. Minden ott rövid távú.",
    recent_boards: "Recent Boards",
    rectangle: "Téglalap",
    share_instructions:
      "Ha valakivel valós időben szeretne együttműködni egy rajzon, küldje el neki az URL-jét.",
    size: "Méret",
    square: "Négyzet",
    straight_line: "Egyenes vonal",
    tagline:
      "Ingyenes és nyílt forráskódú online együttműködési rajzoló eszköz. Vázoljon fel új ötleteket a WBO-n!",
    text: "Szöveg",
    tools: "Eszközök",
    view_source: "Forráskód a GitHub-on",
    zoom: "Nagyítás/kicsinyítés",
  },
  it: {
    board_name_placeholder: "Nome della lavagna…",
    circle: "Cerchio",
    click_to_toggle: "Fai Clic per attivare",
    click_to_zoom:
      "Fai clic per ingrandire \nPremi [MAIUSC] e fai clic per ridurre",
    collaborative_whiteboard: "Lavagna collaborativa",
    color: "Colore",
    configuration: "Configurazione",
    create_private_board: "Crea una lavagna privata",
    ellipse: "Ellisse",
    eraser: "Gomma",
    grid: "Griglia",
    hand: "Mano",
    index_title: "Benvenuti a WBO!",
    introduction_paragraph:
      'WBO è una lavagna collaborativa online <a href="https://github.com/lovasoa/whitebophir" title="gratuita come é gratuita la libertà di espressione, no come un boccale di birra gratis. Questo software è rilasciato sotto licenza AGPL">gratuita e open source</a> che consente a molti utenti di disegnare contemporaneamente su una grande lavagna virtuale. La lavagna viene aggiornata in tempo reale per tutti gli utenti connessi e lo stato è sempre persistente. Può essere utilizzato per molti scopi diversi, tra cui arte, intrattenimento, design e insegnamento.',
    keyboard_shortcut: "scorciatoia da tastiera",
    loading: "Caricamento in corso",
    menu: "Menu",
    mousewheel: "rotella del mouse",
    mover: "Spostamento",
    named_private_board_description:
      "Puoi anche creare una <strong>lavagna privata con un nome creato da te</strong>, con un URL personalizzato, che sarà accessibile a tutti coloro che ne conoscono il nome.",
    opacity: "Opacità",
    open_public_board: "Vai alla lavagna pubblica",
    pencil: "Matita",
    private_board_description:
      "Puoi creare una <b>lavagna privata</b> con un nome casuale, che sarà accessibile solo dal suo URL. Usalo se vuoi condividere informazioni private.",
    public_board_description:
      "La <b>lavagna pubblica</b> è accessibile a tutti. È un disastro felicemente disorganizzato dove puoi incontrare sconosciuti anonimi e disegnare insieme. Tutto in questo spazio è effimero.",
    recent_boards: "Lavagne recenti",
    rectangle: "Rettangolo",
    share_instructions:
      'Per collaborare a un disegno in tempo reale con qualcuno, basta condividere l\'<abbr title="un link tipo https://wbo.ophir.dev/boards/il-codice-della-tua-lavagna">URL della lavagna</abbr>.',
    size: "Dimensione",
    square: "Quadrato",
    straight_line: "Linea retta",
    tagline:
      "Uno strumento collaborativo per disegnare online, gratuito e open source. Disegniamo insieme nuove idee su WBO!",
    text: "Testo",
    tools: "Strumenti",
    view_source: "Codice sorgente su GitHub",
    "white-out": "Bianchetto",
    zoom: "Zoom",
  },
  ja: {
    board_name_placeholder: "ボードの名前",
    circle: "サークル",
    click_to_toggle: "クリックして切り替えます",
    click_to_zoom: "クリックで拡大\nシフトを押しながらクリックで縮小",
    collaborative_whiteboard: "協同作業できるオンラインホワイトボード",
    color: "色",
    configuration: "設定",
    create_private_board: "プライベートボードを作成する",
    ellipse: "楕円",
    eraser: "消去",
    grid: "グリッド",
    hand: "手のひらツール",
    index_title: "WBOへようこそ!",
    introduction_paragraph:
      'WBOは<a href="https://github.com/lovasoa/whitebophir" title="ビール飲み放題ではなく言論の自由。このソフトウェアはAGPLライセンスで公開しています。">無料かつオープンソース</a>の協同作業できるオンラインホワイトボードです。多くのユーザーが大きな仮想ホワイトボードに図などを書くことができ、接続しているすべてのユーザーの更新をリアルタイムに反映され、その状態を常に保存します。これはアート、エンタテインメント、デザインや教育など、様々な用途で使用できます。',
    keyboard_shortcut: "キーボードショートカット",
    loading: "読み込み中",
    menu: "メニュー",
    mousewheel: "ねずみ車",
    mover: "変位",
    named_private_board_description:
      "<strong>名前つきプライベートボード</strong>を作ることもできます。このボードは名前かURLを知っている人だけがアクセスできます。",
    opacity: "透明度",
    open_public_board: "公開ボードを作成する",
    pencil: "ペン",
    private_board_description:
      "プライベートな情報を共有したいときは、ランダムな名前を持つ、<b>プライベートボード</b>を作成できます。このボードはリンクを知っている人がアクセスできます。",
    public_board_description:
      "<b>公開ボード</b>は、WBOにアクセスできる人であれば誰でも参加できますが、これは一時的な用途に向いています。",
    recent_boards: "Recent Boards",
    rectangle: "矩形",
    share_instructions: "URLを送るだけで、リアルタイムな共同作業ができます。",
    size: "サイズ",
    square: "正方形",
    straight_line: "直線",
    tagline:
      "無料でオープンソースの協同作業できるオンラインホワイトボード。WBOでアイディアを共有しましょう!",
    text: "テキスト",
    tools: "ツール",
    view_source: "GitHubでソースコード見る",
    "white-out": "修正液",
    zoom: "拡大・縮小",
  },
  pt: {
    "White-out": "Branquejar",
    board_name_placeholder: "Nome do quadro...",
    circle: "Círculo",
    click_to_toggle: "clique para alternar",
    click_to_zoom:
      "Clique para aumentar o zoom\nPressione shift e clique para diminuir o zoom",
    collaborative_whiteboard: "Quadro branco colaborativo",
    color: "Cor",
    configuration: "Configuração",
    create_private_board: "Crie um quadro privado",
    ellipse: "Elipse",
    eraser: "Borracha",
    grid: "Grade",
    hand: "Mão",
    index_title: "Bem-vindo ao quadro branco online gratuito WBO!",
    introduction_paragraph:
      'WBO é um quadro branco colaborativo online <a href="https://github.com/lovasoa/whitebophir" title="Free as in free speech, não uma cerveja gratuita. Este software é lançado sob a licença AGPL"> livre e de código aberto </a> que permite que muitos usuários desenhem simultaneamente em um grande quadro virtual. O quadro é atualizado em tempo real para todos os usuários conectados e seu estado é sempre persistido. Ele pode ser usado para muitos propósitos diferentes, incluindo arte, entretenimento, design e ensino.',
    keyboard_shortcut: "atalho de teclado",
    loading: "Carregando",
    menu: "Menu",
    mousewheel: "roda do mouse",
    mover: "Mover",
    named_private_board_description:
      "Você também pode criar um <strong>quadro privado com nome</strong>, uma URL personalizado, que estará acessível a todos aqueles que sabem seu nome.",
    opacity: "Opacidade",
    open_public_board: "Vá para o quadro público",
    pencil: "Lápis",
    private_board_description:
      "Você pode criar um <b> fórum privado </b> com um nome aleatório, que será acessível apenas por seu link. Use-o se quiser compartilhar informações privadas.",
    public_board_description:
      "O <b> quadro público </b> está acessível a todos. É uma bagunça felizmente desorganizada, onde você pode se encontrar com estranhos anônimos e se reunir. Tudo ali é efêmero.",
    recent_boards: "Recent Boards",
    rectangle: "Retângulo",
    share_instructions:
      "Para colaborar em um desenho em tempo real com alguém, basta compartilhar sua URL.",
    size: "Tamanho",
    square: "Quadrado",
    straight_line: "Linha reta",
    tagline:
      "Uma ferramenta de desenho colaborativo online gratuita e de código aberto. Esboce novas ideias juntos no WBO!",
    text: "Texto",
    tools: "Ferramentas",
    view_source: "Código-fonte no GitHub",
    zoom: "Zoom",
  },
  ru: {
    board_name_placeholder: "Название доски",
    circle: "Круг",
    click_to_toggle: "нажмите, чтобы переключиться",
    collaborative_whiteboard: "Онлайн доска для совместного рисования",
    color: "Цвет",
    configuration: "Настройки",
    create_private_board: "Создать приватную доску",
    ellipse: "Эллипс",
    eraser: "Ластик",
    grid: "Сетка",
    hand: "Рука",
    index_title: "Добро пожаловать на WBO !",
    introduction_paragraph:
      'WBO это бесплатная и <a href="https://github.com/lovasoa/whitebophir" title="открытый исходный код">открытая</a> виртуальная онлайн доска, позволяющая рисовать одновременно сразу нескольким пользователям. С WBO вы сможете рисовать, работать с коллегами над будущими проектами, проводить онлайн встречи, подкреплять ваши обучающие материалы и даже пробовать себя в дизайне. WBO доступен без регистрации.',
    keyboard_shortcut: "горячая клавиша",
    loading: "Загрузка",
    menu: "Панель",
    mousewheel: "колёсико мыши ",
    mover: "Сдвинуть объект",
    named_private_board_description:
      "Также можно создать <b>именную приватную доску</b> которая будет доступна всем тем, кому вы отправили название вашей доски.",
    opacity: "Непрозрачность",
    open_public_board: "Открыть анонимную доску",
    pencil: "Карандаш",
    private_board_description:
      "<b>Приватная доска</b> обладает тем же функционалом, что и анонимная доска. Разница в том, что приватную доску могут видеть только те пользователи, у которых на нее есть ссылка. Используйте приватную онлайн доску в рабочих целях, проводите онлайн уроки, рисуйте с детьми или друзьями. Другие пользователи не смогут удалять или менять ваши работы без вашего разрешения.",
    public_board_description:
      "<b>Анонимная доска</b> позволяет рисовать вместе онлайн. Используйте этот формат для творчества и кучи разных идей. Вдохновляйтесь уже существующими рисунками, дополняйте их и создавайте совместные работы с другими посетителями. Любой пользователь может удалять уже существующие элементы и рисунки.",
    recent_boards: "Недавние доски",
    rectangle: "Прямоугольник",
    share_instructions:
      "Использовать платформу для совместного творчества очень просто. Достаточно поделиться ссылкой URL с теми, кто хочет с вами порисовать. Как только они получат URL они смогут к вам присоединиться.",
    size: "Размер",
    square: "Квадрат",
    straight_line: "Прямая линия",
    tagline:
      "Бесплатная и открытая доска для совместной работы в интернете. Рисуете свои идеи вместе в WBO !",
    text: "Текст",
    tools: "Инструменты",
    view_source: "Исходный код на GitHub",
    "white-out": "Корректор",
    zoom: "Лупа",
  },
  uk: {
    "White-out": "Коректор",
    board_name_placeholder: "Назва дошки…",
    circle: "Коло",
    click_to_toggle: "клацніть, щоб перемкнути",
    click_to_zoom:
      "Клацніть для збільшення\nНатисніть shift та клацініть для зменшення",
    collaborative_whiteboard: "Онлайн дошка для спільної роботи",
    color: "Колір",
    configuration: "Налаштування",
    create_private_board: "Створити особисту дошку",
    ellipse: "Еліпс",
    eraser: "Ґумка",
    grid: "Сітка",
    hand: "Рука",
    index_title: "Вітаємо у відкритій онлайн дошці WBO!",
    introduction_paragraph:
      'WBO це <a href="https://github.com/lovasoa/whitebophir" title="Free as in free speech, not free beer. This software is released under the AGPL license">безкоштовна та відкрита</a> онлайн дошка для спільної роботи, яка дозволяє багатьом користувачам одночасно писати на великій віртуальній дошці. Дошка оновлюється в реальному часі для всіх приєднаних користувачів, та її стан постійно зберігається. Вона може застосовуватись з різною метою, включаючи мистецтво, розваги, дизайн та навчання.',
    keyboard_shortcut: "швидкі клавіші",
    loading: "Завантаження",
    menu: "Меню",
    mousewheel: "коліщатко миші",
    mover: "Пересунути",
    named_private_board_description:
      "Ви також можете створити <strong>особисту дошку з назвою</strong>, з власним URL, яка буде доступна всім, хто знає її назву.",
    opacity: "Прозорість",
    open_public_board: "Перейти до публічної дошки",
    pencil: "Олівець",
    private_board_description:
      "Ви можете створити <b>особисту дошку</b> з випадковою назвою, яка буде доступна лише за відповідним посиланням. Користуйтесь нею, якшо Вам потрібно ділитись особистою інформацією.",
    public_board_description:
      "<b>Публічна дошка</b> доступна для всіх. Там панує повний безлад, де Ви можете зустріти анонімних незнайомців та малювати разом. Тут все ефемерне.",
    recent_boards: "Recent Boards",
    rectangle: "Прямокутник",
    share_instructions:
      "Для спільної роботи на дошці досить повідомити іншій особі адресу URL.",
    size: "Розмір",
    square: "Квадрат",
    straight_line: "Пряма лінія",
    tagline:
      "Безкоштовний онлайн засіб для спільного малювання з відкритим кодом. Разом накресліть нові ідеї у WBO!",
    text: "Текст",
    tools: "Засоби",
    view_source: "Вихідний код на GitHub",
    zoom: "Лупа",
  },
  vn: {
    "White-out": "Trắng",
    board_name_placeholder: "Tên bản …",
    circle: "Tròn",
    click_to_toggle: "Bật/tắt",
    click_to_zoom: "Nhấp để phóng to \n Nhấn shift và nhấp để thu nhỏ",
    collaborative_whiteboard: "Bảng trắng cộng tác",
    color: "Màu",
    configuration: "Cấu hình",
    create_private_board: "Tạo một bảng riêng",
    ellipse: "Hình elip",
    eraser: "Tẩy",
    grid: "Grid",
    hand: "Tay",
    index_title: "Chào mừng bạn đến với WBO bảng trắng trực tuyến miễn phí!",
    introduction_paragraph:
      'WBO là một <a href="https://github.com/lovasoa/whitebophir" title="Miễn phí như trong tự do ngôn luận, không phải bia miễn phí. Phần mềm này được phát hành theo giấy phép AGPL">Miễn phí và open-source</a> cho phép nhiều người dùng vẽ đồng thời trên một bảng ảo lớn. Bảng này được cập nhật theo thời gian thực cho tất cả người dùng được kết nối và trạng thái luôn tồn tại. Nó có thể được sử dụng cho nhiều mục đích khác nhau, bao gồm nghệ thuật, giải trí, thiết kế và giảng dạy.',
    keyboard_shortcut: "Phím tắt",
    loading: "Đang tải",
    menu: "Menu",
    mousewheel: "Lăn chuột",
    mover: "Di chuyển",
    named_private_board_description:
      "Bạn cũng có thể tạo <strong> bảng riêng được đặt tên </strong>, với URL tùy chỉnh, tất cả những người biết tên của nó đều có thể truy cập được.",
    opacity: "Độ Mờ",
    open_public_board: "Đi tới bảng công khai",
    pencil: "Gạch ngang",
    private_board_description:
      "Bạn có thể tạo một <b> bảng riêng </b> với một tên ngẫu nhiên, chỉ có thể truy cập được bằng liên kết của nó. Sử dụng cái này nếu bạn muốn chia sẻ thông tin cá nhân.",
    public_board_description:
      "Tất cả mọi người đều có thể truy cập <b> bảng công khai </b>. Đó là một mớ hỗn độn vô tổ chức vui vẻ, nơi bạn có thể gặp gỡ những người lạ vô danh và cùng nhau vẽ. Mọi thứ ở đó là phù du.",
    recent_boards: "Recent Boards",
    rectangle: "Chữ nhật",
    share_instructions:
      "Để cộng tác trên một bản vẽ trong thời gian thực với ai đó, chỉ cần gửi cho họ URL của bản vẽ đó.",
    size: "Size",
    square: "Vuông",
    straight_line: "Đường thẳng",
    tagline:
      "Một công cụ vẽ cộng tác trực tuyến miễn phí và mã nguồn mở. Cùng nhau phác thảo những ý tưởng mới trên WBO!",
    text: "Text",
    tools: "Công cụ",
    view_source: "Source code on GitHub",
    zoom: "Zoom",
  },
  zh: {
    board_name_placeholder: "白板名称",
    circle: "圈",
    click_to_toggle: "单击以切换",
    click_to_zoom: "点击放大。\n保持班次并单击缩小。",
    collaborative_whiteboard: "在线协作式白板",
    color: "颜色",
    configuration: "刷设置",
    create_private_board: "创建私人白板",
    ellipse: "椭圆",
    eraser: "橡皮",
    grid: "格",
    hand: "移动",
    index_title: "欢迎来到 WBO！",
    introduction_paragraph:
      ' WBO是一<a href="https://github.com/lovasoa/whitebophir">个免费的</a>、开源的在线协作白板，它允许许多用户同时在一个大型虚拟板上画图。该板对所有连接的用户实时更新，并且始终可用。它可以用于许多不同的目的，包括艺术、娱乐、设计和教学。',
    keyboard_shortcut: "键盘快捷键",
    loading: "载入中",
    menu: "目录",
    mousewheel: "鼠标轮",
    mover: "平移",
    named_private_board_description:
      "您还可以创建一个命名的私有白板，它有一个自定义的URL，所有知道它名字的人都可以访问它。",
    opacity: "不透明度",
    open_public_board: "进入公共白板",
    pencil: "铅笔",
    private_board_description:
      "您可以创建一个带有随机名称的私有白板，该白板只能通过其链接访问。如果要共享私人信息，请使用此选项。",
    public_board_description:
      "每个人都可以使用公共白板。这是一个令人愉快的混乱的地方，你可以会见匿名陌生人，并在一起。那里的一切都是短暂的。",
    recent_boards: "Recent Boards",
    rectangle: "矩形",
    share_instructions: "要与某人实时协作绘制图形，只需向他们发送白板的URL。",
    size: "尺寸",
    square: "正方形",
    straight_line: "直线",
    tagline: "打开即用的免费在线白板工具",
    text: "文本",
    tools: "工具",
    view_source: "GitHub上的源代码",
    "white-out": "修正液",
    zoom: "放大",
  },
};
